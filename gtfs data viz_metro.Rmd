...
The GTFS dataset contains information for both bus, metro/subway, LRT, commuter rail or/and other public transport. In this analysis, we need obtain the stops and routes which belong to Metro. 

The Metro stops and routes are characterized as follows:
...
Then, get colour for metro only. The code creates `shapes_colors_metro`, a filtered version of `shapes_colors` that includes only shape IDs associated with Metro routes. It does this by first identifying `shape_id`s that belong to trips where the `route_id` is in `routes_metro`, ensuring that only Metro-related shapes are retained. 

The `unique()` function removes any duplicate rows that may exist after the filtering process. Finally, the dataset is arranged by `shape_id` to maintain an organized structure, making it easier to analyze and visualize Metro route colors systematically.
